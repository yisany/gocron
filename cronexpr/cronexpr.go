...
// Package cronexpr parses cronexpr time expressions.
package cronexpr
...
// A Expression represents a specific cronexpr time expression as defined at
...
// MustParse returns a new Expression pointer. It expects a well-formed cronexpr
// expression. If a malformed cronexpr expression is supplied, it will `panic`.
// See <https://github.com/gorhill/cronexpr#implementation> for documentation
// about what is a well-formed cronexpr expression from this library's point of
// view.
...
// cronexpr expression is supplied.
// See <https://github.com/gorhill/cronexpr#implementation> for documentation
// about what is a well-formed cronexpr expression from this library's point of
// view.
...
// matches the cronexpr expression `expr`.
//
...
func (expr *Expression) Next(fromTime time.Time) time.Time {
	...
	// supplied time stamp is a perfect match to the underlying cronexpr
	// expression, and since this function is an entry point where `fromTime`
	// does not necessarily matches the underlying cronexpr expression,
	// we first need to ensure supplied time stamp matches
	// the cronexpr expression. If not, this means the supplied time
...
// `fromTime` which match the cronexpr expression `expr`.
//
