...
func TestAt(t *testing.T) {
	...
	nextRun := dayJob.GetScheduledDateTime()
...
func TestWeekdays(t *testing.T) {
	...
	t.Logf("job1 scheduled for %s", job1.GetScheduledDateTime())
	t.Logf("job2 scheduled for %s", job2.GetScheduledDateTime())
	assert.NotEqual(t, job1.GetScheduledDateTime(), job2.GetScheduledDateTime(), "Two jobs scheduled at the same time on two different weekdays should never run at the same time")
}
...
func TestWeekdaysTodayAfter(t *testing.T) {
	...
	t.Logf("job is scheduled for %s", job.GetScheduledDateTime())
	if job.GetScheduledDateTime().Weekday() != timeToSchedule.Weekday() {
	...
	if !job.GetScheduledDateTime().Equal(nextWeek) {
		t.Errorf("Job should be scheduled for the correct time next week.\nGot %+v, expected %+v", job.GetScheduledDateTime(), nextWeek)
	}
}
...
func TestWeekdaysTodayBefore(t *testing.T) {
	...
	job, _ := getWeekday(scheduler.Every(1), time.UTC).At(runTime).Do(task)
	t.Logf("job is scheduled for %s", job.GetScheduledDateTime())
	if !job.GetScheduledDateTime().Equal(timeToSchedule) {
